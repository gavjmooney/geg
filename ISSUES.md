...
- [x] New metrics (the "1–2 additions" flagged in `library_update_brief.md`) — **Node-Edge Occlusion** is now in (`geg.node_edge_occlusion`). Definition is a cubic soft-overlap penalty on the gap between an edge and a non-incident node's drawn glyph: `max(0, 1 - gap / ε) ** 3`. Default `epsilon_fraction = 0.02` (lowered from the initial 0.03 to account for the glyph buffer). The footprint is now **shape-aware**: an explicit `radius` wins (disk); otherwise a `shape` ∈ {square, rectangle, rect} node is modelled as the axis-aligned box it is drawn as (true segment-to-rectangle gap, no circumscribed-disk over-reporting), and ellipse/circle (or dimensioned-but-untagged) nodes use the circumscribing disk. Nodes with no radius/dimensions/shape get a disk of `fallback_radius_fraction * bbox_diagonal` (default 0.01; pass 0.0 for the old centre-to-line behaviour). **TVCG-impact: none** — NEO is new, not part of the published dataset. **Formula confirmed and finalised.**
